...
- Optional OSINT identity adapters (Holehe, PhoneInfoga, Mosint, h8mail) are free CLI wrappers only. Missing binaries soft-fail. Optional CourtListener REST v4 is a free public RECAP/docket search (`Authorization: Token …`); token missing, 429, or errors soft-fail as `UNAVAILABLE` and do not raise risk. Do not add paid Hunter/HIBP/DeHashed/IntelX/Epieos keys and do not fetch paid PACER documents. Never persist breach passwords; redact credential pairs before notes or evidence. OSINT must not contact claimants.
