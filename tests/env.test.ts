...
describe("local .env loader",()=>{
  ...
  it("ignores comments and blank lines",()=>{
    ...
    expect(parsed.BITDEER_API_KEY).toBe("abc");
  });
});
...
describe("optional direct-search env",()=>{
  ...
  it("trims blanks in the toolkit CSV",()=>{expect(parseEnv({COMPOSIO_TOOLKITS:"tavily, ,exa,,steel"}).COMPOSIO_TOOLKITS).toEqual(["tavily","exa","steel"]);});
});

describe("HubSpot production allowlist",()=>{
  it("accepts mixed GUID and name configuration",()=>{expect(hubspotAllowlistConfigured({HUBSPOT_INITIAL_FORM_ID:"guid-initial",HUBSPOT_EMAIL_FORM_NAME:"Email Followup"})).toBe(true);});
  it("rejects a missing side of the two-form allowlist",()=>{expect(hubspotAllowlistConfigured({HUBSPOT_INITIAL_FORM_ID:"guid-initial"})).toBe(false);});
});

describe("HubSpot sync mode selection",()=>{
  it("defaults auto to crm_notes when the form allowlist is not configured",()=>{expect(resolveHubSpotSyncMode({HUBSPOT_SYNC_MODE:"auto"})).toBe("crm_notes");expect(parseEnv({}).HUBSPOT_SYNC_MODE).toBe("auto");});
  it("defaults auto to forms when both allowlist sides are present",()=>{expect(resolveHubSpotSyncMode({HUBSPOT_SYNC_MODE:"auto",HUBSPOT_INITIAL_FORM_ID:"guid-initial",HUBSPOT_EMAIL_FORM_ID:"guid-email"})).toBe("forms");});
  it("honors an explicit crm_notes mode even if form GUIDs are set",()=>{expect(resolveHubSpotSyncMode({HUBSPOT_SYNC_MODE:"crm_notes",HUBSPOT_INITIAL_FORM_ID:"guid-initial",HUBSPOT_EMAIL_FORM_ID:"guid-email"})).toBe("crm_notes");});
  it("honors an explicit forms mode",()=>{expect(resolveHubSpotSyncMode({HUBSPOT_SYNC_MODE:"forms"})).toBe("forms");});
  it("does not require the two-form allowlist for CRM-note production",()=>{expect(()=>assertHubSpotProductionConfig({HUBSPOT_ACCESS_TOKEN:"pat-token",HUBSPOT_SYNC_MODE:"auto"})).not.toThrow();expect(()=>assertHubSpotProductionConfig({HUBSPOT_ACCESS_TOKEN:"pat-token",HUBSPOT_SYNC_MODE:"crm_notes"})).not.toThrow();});
  it("still requires the two-form allowlist when forms mode is selected",()=>{expect(()=>assertHubSpotProductionConfig({HUBSPOT_ACCESS_TOKEN:"pat-token",HUBSPOT_SYNC_MODE:"forms"})).toThrow(/HUBSPOT_TWO_FORM_ALLOWLIST_REQUIRED/);});
  it("still requires the access token in either mode",()=>{expect(()=>assertHubSpotProductionConfig({HUBSPOT_SYNC_MODE:"crm_notes"})).toThrow(/HUBSPOT_ACCESS_TOKEN_REQUIRED/);});
});
