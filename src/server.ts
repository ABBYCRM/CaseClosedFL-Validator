...
await app.register(statik,{root:path.resolve("public"),prefix:"/admin/"});
await routes(app);
app.setErrorHandler((error:any,_req:any,reply:any)=>{app.log.error(error);reply.code(500).send({error:"INTERNAL_ERROR"});});
const stopHubSpot=startHubSpotWorker((obj,msg)=>app.log.info(obj,msg));
