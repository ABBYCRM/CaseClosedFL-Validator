...
`API -> schema -> deterministic intake hard-stops -> jurisdiction/case skill -> source registry -> Composio read-only tools (or direct keyed fallbacks) -> signed ScreenshotOne capture of official URLs -> optional NVIDIA vision OCR (observed text only) -> evidence ledger -> optional NVIDIA document extraction -> deterministic decision -> signed/hashable JSON result`
