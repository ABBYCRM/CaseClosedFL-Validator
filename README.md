...
Optional free OSINT identity lookups (`IDENTITY_OSINT_LOOKUP`) run **inside** the existing validation/fraud path when `OSINT_IDENTITY_ENABLED=true`: Holehe (email registrations), PhoneInfoga (phone), Mosint (email recon), and h8mail (local/free breach). When `COURTLISTENER_ENABLED=true`, the same path adds one free CourtListener REST v4 RECAP/docket search (`Authorization: Token <token>`). The production `Dockerfile` (DigitalOcean App Platform) bakes those four CLIs onto `PATH`; missing or failing CLIs still soft-fail as `UNAVAILABLE`. CourtListener token/rate-limit/errors are also `UNAVAILABLE` and do not raise risk. Paid APIs (Hunter, HIBP, DeHashed, IntelX, Epieos) and paid PACER fetches are out of scope. HubSpot notes lead with a staff verdict (🟢 GOOD / 🟡 CAUTION / 🔴 RED FLAG / ⚪ INCOMPLETE) plus contact and OSINT blocks — missing tools are never treated as risk. CourtListener notes state that this is a public court-records signal only, not a criminal background check. See `docs/OSINT_IDENTITY_SETUP.md`.
